...
class IV(object):
    ...
    def scan_IinVinVout_CURR(self, file_name, max_Iin, inputPolarity, steps, stepSize, Vref, Voff):
        ...
        with open(file_name, 'wb') as outfile:
            ...
            f.writerow(['Input current [A]', 'Input voltage [V]', 'output voltage [V]', 'output-reference ratio [1]', 'Temp-Sensor 1', 'Temp-Sensor 2'])
            ...
            for x in range(0, int(steps)):
                ...
                #
                if inputCurr == 0 or float(inputCurr)+2*float(stepSize) > float(max_Iin):
                ...
                misc.data.append([input_current, input_voltage, output_voltage_1, outputRefRatio, temperature_1, temperature_2])
    ...
    def livePlot(self, file_name):
        ...
        plt.legend()
        plt.savefig("output/IV_CURR/manyPlots/"+file_name.split("output/IV_CURR/")[1]+".pdf")
...
#    iv.maximumInputCurrent = 1.0    #1A chip
    ...
    fileName = "output/IV_CURR/IV_CURR_Vref_500mV_Voffs_800mV_Temp_"+temperature+"C.csv"
    ...
    fileName = "output/IV_CURR/IV_CURR_Vref_600mV_Voffs_600mV_Temp_"+temperature+"C.csv"
    
    #scan_IinVinVout_CURR(self, file_name, max_Iin, inputPolarity, steps, stepSize, Vref, Voff)
    iv.scan_IinVinVout_CURR(    fileName,  1.99,    1,             100,   0.02,     0.6,  0.6)
    
    fileName = "output/IV_CURR/IV_CURR_Vref_600mV_Voffs_800mV_Temp_"+temperature+"C.csv"
    
    #scan_IinVinVout_CURR(self, file_name, max_Iin, inputPolarity, steps, stepSize, Vref, Voff)
    iv.scan_IinVinVout_CURR(    fileName,  1.99,    1,             100,   0.02,     0.6,  0.8)
    
    fileName = "output/IV_CURR/IV_CURR_Vref_600mV_Voffs_1000mV_Temp_"+temperature+"C.csv"
    
    #scan_IinVinVout_CURR(self, file_name, max_Iin, inputPolarity, steps, stepSize, Vref, Voff)
    iv.scan_IinVinVout_CURR(    fileName,  1.99,    1,             100,   0.02,     0.6,  1.0)
    
    fileName = "output/IV_CURR/IV_CURR_Vref_600mV_Voffs_1200mV_Temp_"+temperature+"C.csv"
    
    #scan_IinVinVout_CURR(self, file_name, max_Iin, inputPolarity, steps, stepSize, Vref, Voff)
    iv.scan_IinVinVout_CURR(    fileName,  1.99,    1,             100,   0.02,     0.6,  1.2)
