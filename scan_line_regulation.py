...
class IV(object):    
    ...
    def scan_IinVinVout_CURR(self, file_name, max_Iin, polarity, steps , stepsize, load, vref, vofs, *device):
        ...
        misc.set_source_mode('VOLT', 2, 'Sourcemeter1')     #Setting this device/channel to supply Vofs or Vref
    ...
    def scan_IinVinVout_VOLT(self, file_name, max_Vin, polarity, steps , stepsize, load, vref, vofs, *device):
        '''
        ...
        with open(file_name, 'wb') as outfile:
            ...
            for x in range(0, int(steps)):                                                                                                                 #Measurement loop                                                                
                dut['Sourcemeter1'].set_voltage(polarity*input, channel=1)       
                input_current = misc.measure_current(1, 'Sourcemeter1')[0]
                input_voltage = misc.measure_voltage(1, 'Sourcemeter1')[0]
                output_voltage_1 = misc.measure_voltage(1, 'Sourcemeter2')[0]
                ...
                if float(input_voltage) >= 1.99 or input_current > 2:                                                                                                #Maximum values reached?
                    break             
    ...
    def LivePlot(self, file_name):
        ...
        if "CURR" in file_name:
            ...
            plt.plot(V_in, V_drop, ".-", markersize=3, linewidth=0.5, color = 'g', label = 'Dropout Voltage')
            plt.axis([0,1.5,0,1.75])
            plt.legend()
            plt.savefig(file_name + '.pdf')
        else:
