...
class IV(object):
    ...
    def scan_loadreg_CURR(self, file_name, Iin, inputPolarity, max_Iload, loadPolarity, steps, stepSize, Vref, Voff):
        ...
        if float(Iin) > float(self.maximumInputCurrent):
            ...
            return
        if float(max_Iload) > float(Iin):
            print "ERROR: max_Iload > Iin"
            return
        ...
        with open(file_name, 'wb') as outfile:
            ...
            f.writerow(['load current [A]', 'Input voltage [V]', 'output voltage [V]', 'output-reference ratio [1]'])
            ...
            for x in range(0, int(steps)):
                ...
                misc.data.append([load_current_1, input_voltage, output_voltage_1, outputRefRatio])
    ...
    def scan_loadreg_VOLT(self, file_name, Vin, inputPolarity, max_Iload, loadPolarity, steps, stepSize, Vref, Voff):
        '''
        ...
        with open(file_name, 'wb') as outfile:
            ...
            for x in range(0, int(steps)):
                ...
                #
                loadCurr += stepSize                                                                            #Increase input current for next iteration
                if float(loadCurr) > float(max_Iload) or float(input_current) >= self.maximumInputCurrent:      #Maximum values reached?
                    break
